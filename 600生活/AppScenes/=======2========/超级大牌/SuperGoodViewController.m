...
#import "SuperGoodViewController.h"
#import "SuperGoodTableViewCell.h"
#import "GoodDetailViewController.h"

@interface SuperGoodViewController ()
...
@implementation SuperGoodViewController
...
-(void)setupBottomTableView
{
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"SuperGoodTableViewCell" bundle:nil] forCellReuseIdentifier:@"SuperGoodTableViewCell"];
    [self addMJRefresh];
}

#pragma mark - 网络请求
-(void)requestActivityGoods_PinPaiWithPageIndex:(NSInteger)pageIndex
{
    NSDictionary* param = @{
        @"page" : [NSNumber numberWithInteger:pageIndex],
        @"page_size" : @"10",
        @"activity_type" : @"pinpai"
      };
      
      [self PostWithUrlStr:kFullUrl(kGetActivityGoods) param:param showHud:YES resCache:nil success:^(id  _Nullable res) {
          if(kSuccessRes){
              [self ActivityGoods_PinPaiWithPageIndex: pageIndex data:res[@"data"]];
          }
...
-(void)ActivityGoods_PinPaiWithPageIndex:(NSInteger)pageIndex data:(NSDictionary*)data
{
    NSString* bannerUrlStr = data[@"banner"];
    __weak SuperGoodViewController* wself = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        [wself.bgImageView sd_setImageWithURL:[NSURL URLWithString:bannerUrlStr] placeholderImage:[UIImage imageNamed:@"超级大牌_bg"]];
    });
    ...
    for(int i = 0; i < goods_list.count; i++){
        NSError* err = nil;
        SuperGood* superGood = [[SuperGood alloc]initWithDictionary:goods_list[i] error:&err];
        if(superGood){
            [mutArr addObject:superGood];
        } else {
...
{
    SuperGoodTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"SuperGoodTableViewCell" forIndexPath:indexPath];
...
{
    SuperGood* superGood =[self.datasource objectAtIndex:indexPath.row];
    if(superGood.item_id){
        GoodDetailViewController* vc = [[GoodDetailViewController alloc]initWithItem_id:superGood.item_id];
...
-(void)addMJRefresh
{
    ...
    self.tableview.mj_header = [LLRefreshGifHeader headerWithRefreshingBlock:^{
        ...
        //获取评论数据
        wself.pageIndex=1;
        [wself requestActivityGoods_PinPaiWithPageIndex:wself.pageIndex];
        [wself impactLight];
    }];
