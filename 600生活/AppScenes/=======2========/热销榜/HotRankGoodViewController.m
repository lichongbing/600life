...
#import "HotRankGoodViewController.h"
#import "HotRankTableViewCell.h"
#import "HotRankGoodModel.h"
#import "GoodDetailViewController.h"

@interface HotRankGoodViewController ()
...
//是否从高到底排序
@property(nonatomic,assign)BOOL isShowRankDesc;

@end

@implementation HotRankGoodViewController

-(id)initWithIsShowRankDesc:(BOOL)isShowRankDesc
{
    if(self = [super init]){
        self.isShowRankDesc = isShowRankDesc;
    }
    return self;
}
...
-(void)setupBottomTableView
{
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"HotRankTableViewCell" bundle:nil] forCellReuseIdentifier:@"HotRankTableViewCell"];
    [self addMJRefresh];
}
...
-(void)requestHotsaleGoodsWithPageIndex:(NSInteger)pageIndex
{
    if(self.isShowRankDesc){
        ////////////////////////////////////////降序排行请求 无banner //////////////////////////////
        NSDictionary* param = @{
            @"page" : [NSNumber numberWithInteger:pageIndex],
            @"page_size" : @"10",
        };
        __weak HotRankGoodViewController* wself = self;
        [self GetWithUrlStr:kFullUrl(kHotsaleGoods) param:param showHud:YES resCache:nil success:^(id  _Nullable res) {
            if(kSuccessRes){
                //降序排行处理
                [wself handleHotsaleGoodsWithPageIndex:pageIndex data:res[@"data"]];
            }
...
//降序排行处理
-(void)handleHotsaleGoodsWithPageIndex:(NSInteger)pageIndex data:(NSArray*)datas
{
    NSArray* goods_list = datas;
    NSMutableArray* mutArr = [NSMutableArray new];
    for(int i = 0; i < goods_list.count; i++){
        NSError* err = nil;
        HotRankGoodModel* hotRankGoodModel = [[HotRankGoodModel alloc]initWithDictionary:goods_list[i] error:&err];
        if(hotRankGoodModel){
            [mutArr addObject:hotRankGoodModel];
        } else {
...
//活动排行处理
-(void)handleActivityHotsaleGoodsWithPageIndex:(NSInteger)pageIndex data:(NSDictionary*)data
{
    NSString* bannerUrlStr = data[@"banner"];
    __weak HotRankGoodViewController* wself = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        [((UIImageView*)wself.tableview.tableHeaderView) sd_setImageWithURL:[NSURL URLWithString:bannerUrlStr]];
    });
    
    NSArray* goods_list = data[@"goods_list"];
    [self handleHotsaleGoodsWithPageIndex:pageIndex data:goods_list];
}
...
{
    HotRankTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"HotRankTableViewCell" forIndexPath:indexPath];
...
{
    HotRankGoodModel* hotRankGoodModel = [self.datasource objectAtIndex:indexPath.row];
    if(hotRankGoodModel.item_id){
        GoodDetailViewController* vc = [[GoodDetailViewController alloc]initWithItem_id:hotRankGoodModel.item_id];
...
-(void)addMJRefresh
{
    ...
    self.tableview.mj_header = [LLRefreshGifHeader headerWithRefreshingBlock:^{
        ...
        //获取评论数据
        wself.pageIndex=1;
        [wself requestHotsaleGoodsWithPageIndex:wself.pageIndex];
        [wself impactLight];
    }];
